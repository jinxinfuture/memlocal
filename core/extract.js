...
const QUESTION_RE = /[?？]$|[吗么呢吧]$|^请|^帮我|^你帮我|^帮我|^给|^可以|^能不能|^请问|^如何|^怎么|^什么|^为什么|^哪|^多少|^什么时候|^是不是|^有没有|^你(觉得|认为|知道|看|说|听|会|能)/;

// 临时日程/事件：过完就失效，不是长期记忆（如「明天开会」「周末聚餐」）
const EVENT_RE = /(明天|后天|今天|今晚|周末|下周|下个月|待会|稍后|马上|这次|下次|昨天).*(开会|聚餐|吃饭|聚会|见面|出差|旅游|休假|请假|睡觉|起床|加班|通勤)/;
